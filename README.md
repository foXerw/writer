...
**真实进度**: 端到端可用链路为「撰写正文 + 角色/设定 CRUD + 章节重命名 + 大纲导航 + 自动保存（静默）+ 菜单栏动作 + 编辑器设置（设置页可达）+ Tab 右键菜单 + 切章/返回/关窗自动 flush + Markdown 导出」。阶段 16（统计重做）、17 剩余（Word/PDF/ePub 导出）、15（快捷键自定义）仍待做。
