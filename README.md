...
| 17 | 导入导出功能 | 🔄 部分（Markdown 已实现；Word/PDF/ePub 待做） | 🟢 中 |
| 18 | 主题与UI打磨 | ✅ 已实现（字号/换行/行号接编辑器） | 🟢 中 |

**真实进度**: 端到端可用链路为「撰写正文 + 角色/设定 CRUD + 章节重命名 + 大纲导航 + 自动保存（静默）+ 菜单栏动作 + 编辑器设置（设置页可达）+ Tab 右键菜单 + 切章/返回/关窗自动 flush + Markdown 导出 + 写作统计 + 快捷键自定义」。仍待做：17 剩余 Word·PDF·ePub 导出。
