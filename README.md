...
> ⚠️ 下表为 2026-06-24 代码审计后的**真实状态**，详见 [审计报告](docs/AUDIT.md)。此前的「20/20 全部完成」表述经核查**名不副实**，阶段 12/13/16/17 基本为空壳。
...
| 8 | 文件保存与读取 | ⚠️ 部分（自动保存为死代码） | 🔴 最高 |
| 9 | 新建/打开项目功能 | ⚠️ 部分 | 🔴 最高 |
| 10 | 章节CRUD操作 | ⚠️ 部分（重命名损坏） | 🔴 最高 |
| 11 | 章节树与大纲视图 | ⚠️ 部分（大纲组件未渲染） | 🟡 高 |
| 12 | 角色卡片系统 | ❌ 空壳（未接IPC） | 🟡 高 |
| 13 | 世界观设定模块 | ❌ 空壳（未接IPC） | 🟡 高 |
| 14 | 专注模式与打字机模式 | ⚠️ 部分（专注模式不隐藏侧栏） | 🟡 高 |
| 15 | 快捷键与命令面板 | ⚠️ 部分（冲突检测/自定义缺失） | 🟡 高 |
| 16 | 写作统计功能 | ❌ 空壳（mock数据，无图表） | 🟢 中 |
| 17 | 导入导出功能 | ❌ 空壳（无任何导出实现） | 🟢 中 |
| 18 | 主题与UI打磨 | ⚠️ 部分（设置未接编辑器） | 🟢 中 |

**真实进度**: 端到端可用的核心仅「撰写正文」链路（新建/打开项目 → 章节树 → Monaco 编辑器 → 手动保存 → 多 Tab）。阶段 12/13/16/17 为摆设，需接线或重做。
